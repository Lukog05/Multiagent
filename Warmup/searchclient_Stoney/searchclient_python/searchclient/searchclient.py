...
class SearchClient:
    ...
    def main(args: argparse.Namespace) -> None:
        ...
        plan, metrics = search(initial_state, frontier)
        ...
        if plan is None:
            ...
            # Output metrics for masbench in the required format
            if 'explored' in metrics:
                print(f"#Explored: {metrics['explored']}", flush=True)
            if 'generated' in metrics:
                print(f"#Generated: {metrics['generated']}", flush=True)
            if 'alloc' in metrics:
                print(f"#Alloc: {metrics['alloc']:.2f}", flush=True)
